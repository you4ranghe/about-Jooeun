...
| S-05 | `BrowserWindow` — 창틀 · 탭 · 주소창 · 최대화/닫기                  | M    | S-01       |
| S-06 | **아이콘 자리에서 창이 커지는 모션** (FLIP)                         | M    | S-05, S-03 |

#### 창을 붙이며 정한 것 (2026-07-30)

**최소화 버튼을 빼기로 했습니다.** 최소화하면 주소는 `/projects/yutnori` 인데
화면에는 아무것도 없는 상태가 됩니다. 그때 뒤로가기가 무엇을 뜻하는지 설명할 수
없습니다. **누를 수 있어 보이는데 말이 안 되는 버튼**을 두느니 없는 편이 낫습니다.
같은 이유로 시작 버튼도 여전히 그림입니다.

**나머지 버튼은 전부 진짜입니다.** 뒤로·앞으로·새로고침은 브라우저 히스토리를
그대로 쓰고, 최대화는 작업표시줄만 남기고 채웁니다.

**창의 좌표를 들고 다니지 않습니다.** 아이콘이 레이아웃(바탕화면)에 살아 있으므로
창이 뜰 때 `[data-icon="..."]` 를 DOM 에서 찾아 그 자리에서 자라납니다.
닫을 때도 같은 방법으로 그 자리로 돌아갑니다. 주소로 바로 들어와도 똑같이 동작합니다.

**한 겹이 더 늘었습니다.** `RoomShell(방) → ProjectsLayout(바탕화면) → page(창)`.
방이 `/` ↔ `/projects` 에서 살아 있게 한 수법을 그대로 안쪽에 한 번 더 쓴 것입니다.
...
| 5   | **모니터 안 바탕화면** | 완료 | P8. 줌인하면 바탕화면 → 아이콘 더블클릭 → 창이 열립니다                                 |
| 6   | **브라우저 창틀**      | 완료 | S-05·S-06. 아이콘 자리에서 자라고 그 자리로 사라집니다. 작업표시줄에 실행 중 표시       |
| 7   | 프로젝트 폴더 창       | 남음 | S-07. 실제 저장소가 늘면 아이콘이 빽빽해집니다. 그때 폴더로 묶습니다                    |
| 8   | **죽은 코드 정리**     | 완료 | 갤러리 화면이 사라지며 남은 866줄과 폰트 한 벌을 내렸습니다 (아래)                      |
| 9   | **서랍 둘 · 책 한 권 · 램프** | 완료 | 2026-07-31. 장식이던 서랍에 명함과 제작기를, 다섯째 책에 인터페이퍼를 넣었습니다 |
| 10  | **PWA 매니페스트**     | 완료 | 2026-07-31. `display: standalone` 으로 주소창을 없애고 안드로이드 아이콘도 붙였습니다 |
| 11  | **책 모달 포커스 가두기** | 완료 | 2026-08-03. 아래                                                                     |
| 12  | Q-E 밤 그림자          | 완료 | 2026-08-03. (a) 안. 열린 질문 항목 참조                                                |
| 13  | `docs/02` · `03` 갱신  | 남음 | D-01. **초기 커밋 이후 한 번도 안 고쳤습니다** — 아래                                  |

#### 책 모달만 `<div>` 였습니다 (2026-08-03)

`BookModal` 이 이 방에서 **유일하게 `<dialog>` 가 아닌 모달**이었습니다.
`LoginNotice` · `PhoneNotice` 는 이미 `<dialog>` 를 쓰고 주석에 이유까지 적어
뒀는데, 7월 31일에 새로 만들면서 그 규칙이 따라오지 않았습니다.
새 규칙을 정한 것이 아니라 **빠진 한 곳을 맞춘 것**입니다.

`showModal()` 이라야 셋이 한꺼번에 붙습니다 — 포커스가 안에 갇히고, 바깥이
`inert` 가 되고, 덮을 때 **눌렀던 책등으로 포커스가 돌아갑니다.**
...
| 잡은 것                    | 내용                                                                                                     |
| -------------------------- | -------------------------------------------------------------------------------------------------------- |
| 닫는 길을 하나로           | 부모의 `onClose` 를 먼저 부르면 React 가 **열려 있는 dialog 를 떼어 내** 브라우저가 포커스를 되돌릴 곳을 잃습니다. `close()` → `close` 이벤트 → `onClose` 순서라야 합니다 |
| `RoomShell` 의 Esc 사슬    | `if (bookOpen) setBookOpen(false)` 가 바로 그 짓을 하고 있었습니다. 이제 `return` 으로 길만 막고 dialog 에 맡깁니다. 좌우 화살표도 같이 막았습니다 — 안 보이는 이력서 카드가 뒤에서 넘어가면 책을 덮었을 때 다른 장이 나옵니다 |
| 어둡게 덮는 일은 `::backdrop` | `.book` 은 화면을 채우는 투명한 상자가 되고 흐림·어둠은 `::backdrop` 이 맡습니다 (`.notice` 와 같은 모양) |
| `display` 는 `[open]` 에만 | `[open]` 없이 주면 `showModal()` 이 돌기 전 **한 프레임 동안 모달이 아닌 채로** 보입니다                 |

덤으로 하나 나아졌습니다. 최상위 레이어(top layer)에 뜨므로 무대(`.scene`)의
`transform` 과 줌 상태의 `blur` 에 더 이상 딸려가지 않습니다.

#### `docs/02` · `03` 이 다른 프로젝트를 설명하고 있습니다 (2026-08-03 확인)

둘 다 마지막 수정이 `808e800 Initial commit` 입니다. D-01 이 이 작업인데,
갱신이 아니라 **덜어내는 쪽**에 가깝습니다.

| 항목        | 문서가 말하는 것                                        | 실제                                        |
| ----------- | ------------------------------------------------------- | ------------------------------------------- |
| 프로젝트    | `projects` · `tags` · `project_tags` · `project_images` | 테이블 **없음.** `content/projects.ts` 정적 TS |
| 있는 테이블 | 언급 없음                                               | `events` · `resume_items` · `profile`       |
| Storage     | 버킷 · 경로 · 캐시 · 보안 한계 (02 §6)                  | **안 씁니다.** CSS/SVG 만 (`docs/04`)       |
| 원자적 RPC  | `projects` 저장 함수 (02 §7)                            | 없음                                        |
| 라우팅      | 03 §2                                                   | `/calendar` · `/music` · `/login` · `/admin/*` · `/api/weather` 가 전부 빠짐 |
| RLS         | `anon` 읽기 허용 (02 §5.4)                              | `anon` **REVOKE 로 완전 차단**              |

#### 갈아엎고 남은 것 치우기 (2026-07-30)

`/projects` 가 바탕화면이 되면서 **갤러리 목록 화면이 갈 곳을 잃었습니다.**
지워도 화면은 하나도 안 바뀝니다 — 이미 아무 데서도 부르지 않고 있었습니다.

| 내린 것                                        | 줄 수 | 왜                                        |
| ---------------------------------------------- | ----- | ----------------------------------------- |
| `styles/gallery.css`                           | 652   | 창 밖 전체 화면용. 창 안에 그대로 못 씁니다 |
| `components/gallery/GalleryScreen.tsx`         | 93    | `/` · `/projects` 가 더 이상 안 부릅니다  |
| `components/gallery/ProjectBands.tsx`          | 87    | 태그 필터 — 밴드 목록과 함께 사라짐       |
| `getProjectStats()` · `getTagsByFrequency()`   | ~30   | 위 두 화면 전용 함수                      |
| 폰트 Black Han Sans (`--font-heavy`)           | —     | 갤러리 표제 전용. 쓰는 곳 **0곳**         |

**폴더 창(S-07)을 만들 때 되살리지 않고 새로 그립니다.** 저 CSS 는 브라우저 창이
없던 시절 전체 화면을 채우는 전제로 짜여 있어, 창 안에 넣으면 어차피 다시 씁니다.
죽은 채로 들고 있으면 다음에 이 폴더를 여는 사람이 "이게 지금 쓰이나" 부터 확인합니다.

**폰트는 나머지 일곱이 전부 쓰입니다.** 변수별로 실제 참조를 세어 확인했습니다 —
Jua(방) · Plex Sans KR(본문) · Plex Mono(수치·라벨, 38곳) · Nanum Pen(벽 메모) ·
Gothic A1 · Gowun Batang · Nanum Myeongjo(상세페이지 표제 3종).
...
- ~~**Q-E. 밤 조명이 왼쪽에서 오는데 그림자는 왼쪽 아래로 갑니다.**~~ → **해결 (2026-08-03), (a) 안**
  아이패드에 책상 오른쪽을 내주면서 스탠드를 왼쪽 끝으로 옮긴 탓이었습니다.
  낮에는 창(오른쪽)이 광원이라 맞지만, 밤에는 스탠드가 유일한 광원인데
  그림자가 **광원 쪽으로** 누워 있었습니다.

  (b) 스탠드 이동은 안 골랐습니다. 아이패드 뒤 벽쪽으로 올리면 `layout.ts` 의
  `props.lamp` 하나만 고치면 되지만, 스탠드가 책상 물건이 아니라 벽 장식이 되고
  **낮 그림자까지 같이 틀어집니다.** 광원이 시간에 따라 바뀌는 것이 사실이므로
  그림자도 시간에 따라 바뀌는 쪽이 맞습니다.

  부호가 다섯 군데에 흩어져 있던 것을 `scene.css` 의 `--sx`(±1) 하나로 모았습니다.

  | 고친 곳                                  | 무엇                     |
  | ---------------------------------------- | ------------------------ |
  | `.prop svg` · `.thing svg`               | 책상 소품 · 사물의 drop-shadow |
  | `.mon__foot` · `.clock__foot` · `.pad__stand` | 모니터 · 시계 · 아이패드 받침 |

  **`@property` 로 등록했습니다.** 등록하지 않은 커스텀 속성은 전이가 안 걸려
  밤낮이 바뀌는 순간 그림자가 툭 건너뜁니다. `<number>` 로 등록해 두면 -1 → 1 이
  1.1초에 걸쳐 넘어가고, 그동안 그림자가 물건 밑을 쓸고 지나갑니다 —
  램프에 불이 들어오는 것과 같은 시간입니다. `prefers-reduced-motion` 에서는
  0.2초로 줄여 결과만 남깁니다.

  `.curtain` 의 `6px`(오른쪽) 은 그대로 뒀습니다. 창틀에 딸린 벽 장식이라
  책상 위 물건과 같은 광원 규칙을 따르지 않습니다.
